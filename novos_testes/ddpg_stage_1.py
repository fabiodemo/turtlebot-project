...
class Trainer:
    ...
    def save_models(self, episode_count):
        torch.save(self.target_actor.state_dict(), dirPath +'/Models/stage_4/'+str(episode_count)+ '_actor.pt')
        torch.save(self.target_critic.state_dict(), dirPath + '/Models/stage_4/'+str(episode_count)+ '_critic.pt')
        print('****Models saved***')
        
    def load_models(self, episode):
        self.actor.load_state_dict(torch.load(dirPath + '/Models/stage_4/'+str(episode)+ '_actor.pt'))
        self.critic.load_state_dict(torch.load(dirPath + '/Models/stage_4/'+str(episode)+ '_critic.pt'))
        hard_update(self.target_actor, self.actor)
        hard_update(self.target_critic, self.critic)
        print('***Models load***')
    ...
    for ep in range(MAX_EPISODES):
        ...
        for step in range(MAX_STEPS):
            ...
            if ram.len >= 0*MAX_STEPS and is_training:
                var_v = max([var_v*0.9999, 0.30*ACTION_V_MAX])
                var_w = max([var_w*0.9999, 0.30*ACTION_W_MAX])
                #trainer.optimizer()
            if is_training:
                trainer.optimizer()
